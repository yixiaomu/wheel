...
def dated_path(obj, file_data):
    ...
    filename = u"{name}_{rand}{ext}".format(rand=rand, name=parts[0], ext=parts[1])
    filename = secure_filename(filename)
    today = date.today()
    path = u"{prefix}/{t.year}/{t.month}/{filename}".format(prefix=prefix,
                                                            t=today,
                                                            filename=filename)
    return path
...
def media_path(suffix=None):
    ...
    if suffix:
        return os.path.join(media_root, suffix)
